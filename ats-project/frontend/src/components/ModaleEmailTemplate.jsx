...
/**
 * Pannello laterale che mostra i modelli email disponibili per una fase,
 * con i segnaposto già sostituiti dai dati del candidato/posizione.
 *
 * Props:
 *   fase          — fase corrente del candidato (colonna kanban)
 *   candidato     — { first_name, last_name, email, phone, current_role }
 *   posizione     — { titolo }
 *   modelli       — array di tutti i modelli caricati
 *   onChiudi      — callback per chiudere il pannello
 */
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-4"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[88vh] flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Modelli email per la fase <span className={`font-semibold px-1.5 py-0.5 rounded ml-1 ${BADGE_FASE[fase]}`}>{fase}</span>
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          {/* Lista modelli */}
          <div className="w-56 shrink-0 border-r border-slate-100 overflow-y-auto py-3">
            {modelliDellaFase.length === 0 ? (
              ...
              <button key={m.id} onClick={() => { setSelezionato(m); setCopiato(false); }}
                className={`w-full text-left px-5 py-3.5 text-sm transition ${
                  selezionato?.id === m.id
            ...
            {!selezionato ? (
              <div className="flex items-center justify-center h-full text-slate-300">
                <div className="text-center">
                  ...
                  <p className="text-sm">Seleziona un modello</p>
                </div>
              </div>
            ) : (
              <>
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1.5">Destinatario</p>
                  <p className="text-sm text-slate-700">{candidato?.email || <span className="text-slate-300 italic">email non disponibile</span>}</p>
                </div>

                {oggettoCompilato && (
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1.5">Oggetto</p>
                    <p className="text-sm text-slate-800 font-medium">{oggettoCompilato}</p>
                  </div>
                )}

                <div className="flex-1">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1.5">Messaggio</p>
                  <pre className="text-sm text-slate-700 font-sans whitespace-pre-wrap leading-relaxed bg-slate-50 rounded-xl p-4 border border-slate-200">
                    {corpoCompilato}
                  </pre>
                </div>
                ...
                <button onClick={copiaAppunti}
                  ...
                  style={copiato ? {} : { background: 'linear-gradient(135deg,#6366f1,#4f46e5)' }}>
                  {copiato ? (
                    <>
